...
const DarkModeProvider = ({
  ...
  const handleDarkColorScheme = useCallback(
    (    event: { matches: any }) => event.matches && handleChangeColorScheme(colorSchemes.dark),
    [handleChangeColorScheme],
  )

  const handleLightColorScheme = useCallback(
    (    event: { matches: any }) => event.matches && handleChangeColorScheme(colorSchemes.light),
    [handleChangeColorScheme],
  )

  const handleDefaultColorScheme = useCallback(
    (    event: { matches: any }) => event.matches && handleChangeColorScheme(defaultValue),
  ...
  useEffect(() => {
    if (
      ...
      window
        ...
        .addEventListener('change', handleDarkColorScheme)
      window
        .matchMedia(`(prefers-color-scheme: ${colorSchemes.light})`)
        .addEventListener('change', handleLightColorScheme)
      window
        .matchMedia(`(prefers-color-scheme: ${colorSchemes.noPreference})`)
        .addEventListener('change', handleDefaultColorScheme)
      return () => {
        window
          .matchMedia(`(prefers-color-scheme: ${colorSchemes.dark})`)
          .removeEventListener('change', handleDarkColorScheme)
        window
          .matchMedia(`(prefers-color-scheme: ${colorSchemes.light})`)
          .removeEventListener('change', handleLightColorScheme)
        window
          .matchMedia(`(prefers-color-scheme: ${colorSchemes.noPreference})`)
          .removeEventListener('change', handleDefaultColorScheme)
      }
    }
  }, [
    handleDarkColorScheme,
    handleDefaultColorScheme,
  ...
  return (
    <DarkModeContext.Provider value={[colorScheme, handleChangeColorScheme]}>
      {typeof children === 'function' ? children([colorScheme, handleChangeColorScheme]) : children}
    </DarkModeContext.Provider>
  )
}

const useDarkMode = () =>
  useContext<[DarkModeValues, (colorSchema: DarkModeValues) => void]>(DarkModeContext)
